...
namespace QuanlyDancuDothi.DBConnect
{
    public class TrangChuDAO
    {
        ...
        {
            int count;
            string sqlStr = "select GioiTinh, count(MaKS) as SoLuong from KhaiSinh where GioiTinh = 'Nam' group by GioiTinh";
        ...
        {
            int count;
            string sqlStr = "select GioiTinh, count(MaKS) as SoLuong from KhaiSinh where GioiTinh = N'Nữ' group by GioiTinh";
            DataTable dataTable = dB.Sql_Select(sqlStr);
            return count = Convert.ToInt32(dataTable.Rows[0]["SoLuong"]);
        }
        ...
        {
            int count;
            string sqlStr = "select count(MaKS) as SoLuong from KhaiSinh";
            DataTable dataTable = dB.Sql_Select(sqlStr);
            return count = Convert.ToInt32(dataTable.Rows[0]["SoLuong"]);
        }
        ...
        {
            double avg;
            string sqlStr = "select AVG(Tuoi) as TuoiTB from (select datediff(year, NgaySinh, GETDATE()) as Tuoi from CongDan join KhaiSinh on CongDan.MaKS = KhaiSinh.MaKS) Q";
            DataTable dataTable = dB.Sql_Select(sqlStr);
